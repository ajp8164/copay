...
angular.module('copayApp.controllers').controller('appletListController', function($scope, $rootScope, $log, $timeout, $ionicSideMenuDelegate, $ionicSlideBoxDelegate, $ionicModal, lodash, appletService, appletCatalogService, FocusedWallet, Constants, isMobile, isCordova) {
  ...
  function currentLayout() {
    return (lodash.isEmpty(appletService.getActiveCategory()) ? Constants.LAYOUT_LIST : Constants.LAYOUT_CATEGORIES);
  };
  ...
  this.saveApplet = function(oldApplet, newApplet) {
    if (lodash.isEqual(oldApplet, newApplet)) return;
    ...
    if (oldApplet.preferences.category != newApplet.preferences.category) {
      ...
      saveAppletState([newApplet], function() {
        appletService.createCategory(newApplet.preferences.category, function(){
  ...
  var removeWalletAppletUpdated = $rootScope.$on('Local/WalletAppletUpdated', function(event, walletApplet) {
    ...
    var index = lodash.findIndex(self.applets, function(applet) {
      return applet.header.appletId == walletApplet.header.appletId;
    });
    if (index >= 0) {
